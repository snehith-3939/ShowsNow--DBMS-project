...
const SeatLayout = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/seats/${id}`, { headers })
      .then(r => r.json())
      .then(data => {
        setSeats(data);
    ...
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/shows/${id}`).then(r => r.json()).then(setShowInfo);
